...
vector<Publication> parseBibTeX(const string& filePath) {
...
int main() {
    ...
    vector<Publication> publications = parseBibTeX(filePath);
    long avg = 0;
    int count = 0;

    for (const auto& publication : publications) {
        if (publication.hasAuthor(authorName)) {
            cout << colors[count % 6]; // Cycle through colors
            publication.printPublication();
            avg += publication.noOfAuthor();
            count++;
            cout << resetColor; // Reset color after each publication
        }
    }

    if (count > 0) {
        cout << "\nAverage number of co-authors: " << avg / count << endl;
    } else {
        cout << colors[0] << "No publications found for this author." << resetColor << endl;
    }
